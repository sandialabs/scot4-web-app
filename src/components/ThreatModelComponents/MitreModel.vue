...
async mounted(){
  ...
  const resp = await Vue.prototype.$api.elements.retrieveElementsWithFilter(IRElementType.ThreatModelItem, {"_type":"mitre_enterprise", "limit":2000})
  for (const mitreElement of resp.data.result){
      for (const killChainPhaseObj of mitreElement.data.kill_chain_phases){
          if (killChainPhaseObj["kill_chain_name"] == "mitre-attack"){
